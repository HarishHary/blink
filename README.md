...
- Matcher, executor, tuner, and enricher batches retry only pending failures, prepare all outputs before publication, and commit consumed offsets only after synchronous writes.
- Matcher, executor, tuner, and enricher DLQs use protobuf envelopes that retain the failed input's source coordinates, payload, stage, reason, attempts, and timestamp while preserving its Kafka key as message metadata.
