...
func (p *Pool) Format(ctx context.Context, formatterID string, alertBatch []*alerts.Alert, canaryHashKey string) (outs []map[string]any, absent bool, removed bool, errs []errors.Error) {
	// Shadow candidate (if any): separate fan-out at its own max_procs, results dropped.
	p.shadowFormat(ctx, formatterID, alertBatch)
	k := p.ServingPoolSize(formatterID, canaryHashKey)
	parts := pools.ShardConcurrent(alertBatch, k, func(chunk []*alerts.Alert) formatChunkResult {
	...
	outs = make([]map[string]any, 0, len(alertBatch))
	errs = make([]errors.Error, 0, len(alertBatch))
	for _, part := range parts {
...
func (p *Pool) formatChunk(ctx context.Context, formatterID string, chunk []*alerts.Alert, canaryHashKey string) formatChunkResult {
	outs := make([]map[string]any, len(chunk))
	perErrs := make([]errors.Error, len(chunk))
	prodFn := func(callCtx context.Context, f Formatter) error {
		if !f.FormatterMetadata().Enabled {
			return nil
		}
		batchOuts, e := f.Format(callCtx, chunk)
		if e != nil {
			for i := range perErrs {
				perErrs[i] = e
			}
...
func (p *Pool) shadowFormat(ctx context.Context, formatterID string, alertBatch []*alerts.Alert) {
	sk := p.ShadowPoolSize(formatterID)
	if sk == 0 || len(alertBatch) == 0 {
		return
	}
	for _, chunk := range pools.ShardSlice(alertBatch, sk) {
		chunk := chunk
		p.CallShadow(ctx, formatterID, func(callCtx context.Context, f Formatter) error {
			if !f.FormatterMetadata().Enabled {
				return nil
			}
			_, e := f.Format(callCtx, chunk)
			return e
		})
	}
}
