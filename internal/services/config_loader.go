...
// LoadFromEnvironment populates a config struct from environment variables (and files) via
// the `env:`/`file:` struct tags, recursing into nested and embedded structs. A field without
// `,optional` whose env var is empty is an error, so each service's required topics/dirs fail
// fast at load instead of degrading silently at runtime.
