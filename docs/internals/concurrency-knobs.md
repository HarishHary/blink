...
[Internals index](README.md) · [Plugin runtime](plugin-runtime.md) · [Runtime recovery](runtime-recovery.md) · [Event matcher service](../services/event_matcher.md) · [Rule tuner service](../services/rule_tuner.md)
...
- Every deployment keeps its `min_procs`. Growth past those reservations draws on one process-wide `ProcessBudget`, sized `max(GOMAXPROCS x 2, (cgroup memory limit - 256 MiB) / 64 MiB)` and capped at 512. `GOMAXPROCS`, not `NumCPU`, which ignores the container's CPU limit. Memory usually binds first, since these subprocesses idle on gRPC round trips; with no readable cgroup limit the `GOMAXPROCS x 2` floor stands alone.
...
| `ProcessBudget`                                  | derived from CPUs and memory, capped 512                                              | More subprocesses past every `min_procs`, so bursts scale further                                                             | Scale-up stops sooner, closer to the reservations                                                    | Manager queue, paced by the scale cooldown                            |
...
| `HealthInterval`                                 | default 20s                                                                           | Fewer health RPCs competing with invocations                                                                                  | Faster detection of a wedged subprocess                                                              | -                                                                     |
...
| `RestartMin` / `RestartMax` (manager)            | defaults 5s / 5m, 5 scheduled replacements                                            | The manager waits longer before replacing a lost process                                                                      | Faster replacement, finite budget spent sooner                                                       | Queued calls wait; exhaustion opens the circuit                       |
| `PluginProcessOptions.RestartMin` / `RestartMax` | defaults 5s / 5m, 5 scheduled replacements                                            | One process waits longer before restarting its subprocess                                                                     | Faster local restarts, budget spent sooner                                                           | `MessagePluginProcessRestartExhausted`, then the manager replaces it  |
