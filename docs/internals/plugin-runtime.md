...
| `blink_plugin_subprocess_starts_total`, `blink_plugin_subprocess_restarts_total{reason}`, `blink_plugin_subprocess_failures_total`                                                              | plugin process     | Subprocess churn inside one slot, which the process counters above never see. Reason is `failure` for a subprocess that died, `health` for one that stopped answering; a failure is a spent restart budget. |
